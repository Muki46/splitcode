...
function themify_theme_body_class( $classes ) {
	...
	if ( themify_theme_has_split_scroll() ) {
		$classes[] = 'split-scroll full-section-scrolling';
	}
	...
	if ( themify_theme_has_page_loader() ) :
            ...
            if($icon){
                if(strpos($icon,'fa-')===0){
                    ...
                    $bg = 'background:url(' . themify_https_esc( $icon ) . ') center/cover no-repeat';
                }
            }
	?>
        <?php if($color):?>
...
function themify_theme_has_split_scroll() {
    static $is_split = NULL;
    if(is_null($is_split)){
	$is_split = apply_filters( 'themify_theme_has_split_scroll', ( is_search() || !is_page() || themify_is_touch( 'phone' ) || ( isset( $_REQUEST['wp_customize'] ) && 'on' == $_REQUEST['wp_customize'] ) || ( is_admin() && 'customize.php' == basename( $_SERVER['PHP_SELF'] ) ) ) ? false : is_page() && 'yes' == themify_get( 'split_scroll' ) );
    }
    return $is_split;
}
